...
class RBatchNorm(nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-05, momentum=0.1, weight_freeze=False, bias_freeze=False, weight_init=1.0,
                 bias_init=0.0):
        super().__init__(num_features, eps=eps, momentum=momentum)
        self.weight.data = self.weight.data + torch.rand(*self.weight.data.shape)
        self.bias.data = self.bias.data + torch.rand(*self.bias.data.shape)
