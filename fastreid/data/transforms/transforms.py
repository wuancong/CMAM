...
"""
__all__ = ['ToTensor', 'RandomErasing', 'RandomPatch', 'AugMix', 'CREA']
...
class CREA(object):
    ...
    def __call__(self, img):
        ...
        img[:, :, 0] = self.one_forward(img[:, :, 0], 0)
        img[:, :, 1] = self.one_forward(img[:, :, 1], 1)
        img[:, :, 2] = self.one_forward(img[:, :, 2], 2)

        return img

    def one_forward(self, img, idx):
        ...
        for attempt in range(100):
            ...
            if w < img.shape[1] and h < img.shape[0]:
                ...
                img[x1:x1 + h, y1:y1 + w] = self.mean[idx]
                return img
        return img
