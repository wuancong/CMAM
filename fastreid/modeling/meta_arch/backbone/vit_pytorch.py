...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.cfg = cfg
        self.drop = cfg.MODEL.PATCH_DROP
    ...
    def forward_base(self, x):
        ...
        if self.drop > 0:
            P = x.shape[1]
            notdrop = torch.rand((1, P, 1)) >= self.drop
            x = x * notdrop.cuda()
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.attr_num > 0:
            attr_token = self.attr_token.expand(B, -1, -1)
            x = torch.cat((x, attr_token), dim=1)
        x = self.pos_drop(x)
    ...
    def load_param(self, model_path):
        ...
        if self.attr_num > 0:
            ...
            for i in range(self.attr_num):
                self.attr_token.data[:, i, :] = self.cls_token.data[:,0,:]
