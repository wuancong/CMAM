...
class MutualHead(nn.Module):
    def __init__(self, cfg, in_feat, num_classes, pool_layer):
        ...
        #In default, the classifier in cirlce
        self.classifier2 = Circle(cfg, in_feat, num_classes)
    ...
    def forward(self, features, targets=None, bn_precise = False, sub=None):
        ...
        # Training
        pred_class_logits = self.classifier(bn_feat[sub==0], targets[sub==0])
        pred_class_logits2 = self.classifier2(bn_feat[sub==1], targets[sub==1])
        pred_class_logits = torch.cat((pred_class_logits, pred_class_logits2), dim=0)
