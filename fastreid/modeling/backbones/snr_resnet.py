...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, block, layers, non_layers,ltm_enabled = False ):
        self.inplanes = 64
        super().__init__()
        ...
        self.register_buffer('gamma', torch.ones(1, 64 * 4,1,1))
        self.register_buffer('beta', torch.zeros(1, 64 * 4,1,1))
    ...
    def forward(self, x):
        ...
        l1x = x
        x_plus, x_minus, x_ori = self.snr1(x)
        pluss.append( self.avgpool(x_plus).view(x_plus.size(0),-1))
        minuss.append( self.avgpool(x_minus).view(x_minus.size(0),-1))
        oriss.append(self.avgpool(x_ori).view(x_minus.size(0),-1) )
        x = x_plus
        # Layer 2
        NL2_counter = 0
        ...
        x = self.sw2(x)
        l2x = x
        x_plus, x_minus,x_ori = self.snr2(x)
        pluss.append( self.avgpool(x_plus).view(x_plus.size(0),-1))
        minuss.append( self.avgpool(x_minus).view(x_minus.size(0),-1))
        oriss.append(self.avgpool(x_ori).view(x_minus.size(0),-1) )
        x = x_plus
        ...
        x = self.sw3(x)
        l3x = x
        x_plus, x_minus,x_ori = self.snr3(x)
        pluss.append( self.avgpool(x_plus).view(x_plus.size(0),-1))
        minuss.append( self.avgpool(x_minus).view(x_minus.size(0),-1))
        oriss.append(self.avgpool(x_ori).view(x_minus.size(0),-1) )
        x = x_plus
        ...
        x_plus, x_minus,x_ori = self.snr4(x)
        pluss.append( self.avgpool(x_plus).view(x_plus.size(0),-1))
        minuss.append( self.avgpool(x_minus).view(x_minus.size(0),-1))
        oriss.append(self.avgpool(x_ori).view(x_minus.size(0),-1) )
        x = x_plus
        ...
        return x, tuple(pluss), tuple(minuss), tuple(oriss)
...
def build_snr_resnet_backbone(cfg):
    """
