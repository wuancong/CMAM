...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, block, layers, non_layers,ltm_enabled = False, cfg = None ):
        self.inplanes = 64
        super().__init__()
        self.cfg = cfg
        ...
        self.cor_matrix = np.identity(self.gamma.shape[1])
        self.step = 0
    ...
    def add_noise(self, noise_scale ):
        self.gamma.data = torch.ones(1, self.gamma.shape[1], 1,
                                                           1).cuda()
        self.beta.data = torch.zeros(1, self.beta.shape[1], 1,
                                                           1).cuda()
        weight = self.gamma
        # sigma = torch.abs(weight).data
        #sigma is ones()
        # print( 'noise_scale', self.noise_scale )
        N = len(self.cor_matrix)
        noise = np.random.multivariate_normal(np.zeros(N),self.cor_matrix )
        noise = torch.tensor( noise ).float( )
        noise = noise.view( *weight.shape )
        noise = noise * noise_scale
        returned_noise = copy.deepcopy(noise)
        # print( 'gamma',self.gamma.data[0] )
        # print( 'weigh.data', weight.data, noise )
        weight.data = weight.data + noise.cuda( )
        # print( 'weight', weight.data[0], weight.shape )
        if self.step % 100 == 0:
            print( weight.data.mean( ) )
        # assert False
    ...
    def forward(self, x):
        ...
        if self.adv:
            ...
            if self.step >= bound:
                if self.training:
                    self.add_noise( self.noise_scale )
                else:
                    self.add_noise(0.0)
...
def build_swi_resnet_addno2_backbone(cfg):
    """
    ...
    model = ResNet(last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, block,
                   num_blocks_per_stage, nl_layers_per_stage, ltm_enabled, cfg = cfg )
    if pretrain:
        if not with_ibn:
            try:
