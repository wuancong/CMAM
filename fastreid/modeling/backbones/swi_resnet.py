...
model_urls = {
    ...
    50: '/local_home/ancong/.cache/torch/checkpoints/resnet50-19c8e357.pth',
    101: 'https://download.pytorch.org/models/resnet101-5d3b4d8f.pth',
    152: 'https://download.pytorch.org/models/resnet152-b121ed2d.pth',
    'se50': 'http://data.lip6.fr/cadene/pretrainedmodels/se_resnet50-ce0d4300.pth',
}
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, num_splits, with_ibn=False, with_se=False,
        ...
        if with_se:
            self.se_module = SELayer(planes, reduction)
        else:
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, with_ciconv, ciconv_invariant, block, layers, non_layers,ltm_enabled = False ):
        self.inplanes = 64
        super().__init__()
        self.with_ciconv = with_ciconv
        if with_ciconv:
            if ciconv_invariant == 'color_ratio':
                self.ciconv = CRConv2d()
                self.conv1_for_ci = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                                              bias=False)
            elif ciconv_invariant == 'color_ratio_learn':
                self.ciconv = CRConv2d(learn_filter=True)
                self.conv1_for_ci = nn.Identity() #nn.Conv2d(self.ciconv.out_dim, 64, kernel_size=7, stride=2, padding=3, bias=False)
            else:
                self.ciconv = CIConv2d(invariant=ciconv_invariant, k=3, scale=0.0)
                self.conv1_for_ci = nn.Conv2d(self.ciconv.out_dim, 64, kernel_size=7, stride=2, padding=3,
                                              bias=False)
        else:
        ...
        self.sw2 = SwitchNorm2d(128 * 4 )
        self.layer3 = self._make_layer(block, 256, layers[2], 2, bn_norm, num_splits, with_ibn, with_se)
        self.sw3 = SwitchNorm2d(256 * 4)
        self.layer4 = self._make_layer(block, 512, layers[3], last_stride, bn_norm, num_splits, with_se=with_se)
        self.sw4 = SwitchNorm2d(512 * 4 )
        self.adv = False
        self.gamma = nn.Parameter( torch.ones(1, 64 * 4,1,1) )
        self.beta = nn.Parameter( torch.zeros(1, 64 * 4,1,1) )

        # print( self.gamma.data, self.beta.data )
        self.random_init()
    ...
    def forward(self, x):
        ...
        # x_layer1, x_layer2, x_layer3, x_layer4 = l1x, l2x, l3x, l4x
        # x_layer1 = F.adaptive_avg_pool2d(x_layer1, (1, 1))
        # x_layer1 = x_layer1.view(x_layer1.size(0), -1)
        # x_layer1 = x_layer1 / x_layer1.norm(2, 1).unsqueeze(1).expand_as(x_layer1)
        # x_layer2 = F.adaptive_avg_pool2d(x_layer2, (1, 1))
        # x_layer2 = x_layer2.view(x_layer2.size(0), -1)
        # x_layer2 = x_layer2 / x_layer2.norm(2, 1).unsqueeze(1).expand_as(x_layer2)
        # x_layer3 = F.adaptive_avg_pool2d(x_layer3, (1, 1))
        # x_layer3 = x_layer3.view(x_layer3.size(0), -1)
        # x_layer3 = x_layer3 / x_layer3.norm(2, 1).unsqueeze(1).expand_as(x_layer3)
        # x_layer4 = F.adaptive_avg_pool2d(x_layer4, (1, 1))
        # x_layer4 = x_layer4.view(x_layer4.size(0), -1)
        # x_layer4 = x_layer4 / x_layer4.norm(2, 1).unsqueeze(1).expand_as(x_layer4)
...
def build_swi_resnet_backbone(cfg):
    """
    ...
    ciconv_invariant = cfg.MODEL.BACKBONE.CICONV_INVARIANT
    ...
    model = ResNet(last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, with_ciconv, ciconv_invariant, block,
                   num_blocks_per_stage, nl_layers_per_stage, ltm_enabled)
    if pretrain:
        if not with_ibn:
            # try:
            ...
            state_dict = torch.load(model_urls[depth])
            logger.info("Loading pretrained model from torchvision")
        else:
