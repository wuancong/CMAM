...
class RGA_Module(nn.Module):
	...
	def forward(self, x):
		...
		if self.use_spatial:
			...
			theta_xs = theta_xs.permute(0, 2, 1).contiguous( )
			phi_xs = phi_xs.view(b, self.inter_channel, -1)
			Gs = torch.matmul(theta_xs, phi_xs)
			Gs_in = Gs.permute(0, 2, 1).view(b, h*w, h, w).contiguous( )
		...
		if self.use_channel:
			...
			theta_xc = self.theta_channel(xc).squeeze(-1).permute(0, 2, 1).contiguous( )
			phi_xc = self.phi_channel(xc).squeeze(-1)
			Gc = torch.matmul(theta_xc, phi_xc)
			Gc_in = Gc.permute(0, 2, 1).unsqueeze(-1).contiguous( )
