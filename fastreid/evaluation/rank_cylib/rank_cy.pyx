...
cpdef idx_evaluate_cy(indices, q_pids, g_pids, q_camids, g_camids, max_rank, use_metric_cuhk03=False):
    indices = np.asarray(indices, dtype=np.int64)
    q_pids = np.asarray(q_pids, dtype=np.int64)
    g_pids = np.asarray(g_pids, dtype=np.int64)
    q_camids = np.asarray(q_camids, dtype=np.int64)
    g_camids = np.asarray(g_camids, dtype=np.int64)
    return idx_eval_market1501_cy(indices, q_pids, g_pids, q_camids, g_camids, max_rank)
...
cpdef idx_eval_market1501_cy(long[:,:] indices, long[:] q_pids, long[:]g_pids,
                         long[:]q_camids, long[:]g_camids, long max_rank):
