...
def get_test_ids():
    test_id_filepath = "datasets/SYSU-MM01/exp/test_id.mat"
...
def get_cam_permutation_indices(all_cams):
    rand_cam_perm_filepath = "datasets/SYSU-MM01/exp/rand_perm_cam.mat"
...
def evaluate(features, test_ids, settings, total_runs=10):
    ...
    """
    gallery_cams, probe_cams = get_cam_settings(settings)
    all_cams = list(set(gallery_cams + probe_cams))  # get unique cams (1-6)

    # get permutation indices
    cam_permutations = get_cam_permutation_indices(all_cams)
    ...
    for run_index in range(total_runs):
        X_gallery, Y_gallery, cam_gallery, X_probe, Y_probe, cam_probe = get_testing_set(
            features,
            cam_permutations,
            test_ids,
            run_index,
        ...
        dist = euclidean_dist(X_probe, X_gallery)
    ...
    return mean_cmc * 100, mean_mAP * 100


def evaluate_results(features_dict, test_ids, mode='all', number_shot=1, total_runs=10):
    settings = {"mode": mode, "number_shot": number_shot}
    results = evaluate(features_dict, test_ids, settings, total_runs)
    return results
